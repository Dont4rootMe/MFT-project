#!/usr/bin/env python
"""
Main test runner for model validation tests.

This script runs all tests for model output validation including:
...
class ModelTestRunner:
    """Test runner for model validation module."""
    ...
    def run_tests(
        ...
        # Coverage options
        if coverage:
            models_module = self.test_dir.parent
            cmd.extend([
                '--cov=' + str(models_module),
                '--cov-report=term-missing',
                '--cov-report=html:htmlcov_models',
            ])
        
        # HTML report
        if html_report:
            cmd.extend([
                '--html=test_report_models.html',
                '--self-contained-html'
            ])
...
Examples:
  ...
  %(prog)s -f test_models.py      # Run specific test file
  ...
  %(prog)s -m "not slow"          # Custom pytest markers
    ...
    if args.coverage:
        print(f"\nCoverage report saved to: {runner.project_root}/htmlcov_models/index.html")
    
    if args.html_report:
        print(f"HTML test report saved to: {runner.project_root}/test_report_models.html")
